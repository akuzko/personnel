class ShiftLeaderScore < ActiveRecord::Base
  ...
  def self.search_average(params, admin_id)
    ...
    select("avg(score) as avg_score").joins(:shift_leader).where(conditions.join(' and ')).first
  end
  ...
  def self.search_grouped(params, admin_id)
    ...
    select("shift_leader_scores.shift_leader_id, avg(score) as avg_score").
        ...
        paginate :per_page => [params[:per_page].to_i, 5].max, :page => params[:page],
                 :conditions => conditions.join(' and '),
                 :joins => 'INNER JOIN `users` ON `shift_leader_scores`.`shift_leader_id` = `users`.`id` INNER JOIN `profiles` ON `profiles`.`user_id` = `users`.`id`',
                 :order => "#{sort_by[params[:sort_by].to_sym]} #{params[:sort_order]}",
                 :group => '`users`.`id`'
  end
end

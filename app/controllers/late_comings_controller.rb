class LateComingsController < ApplicationController
  ...
  def index
    ...
    @late_comings = LateComing.order('created_at DESC').where('created_at >=?', params[:date_from]).where('created_at <=?', params[:date_to]).find_all_by_user_id(current_user.id).paginate :page => params[:page], :order => 'created_at DESC'
